...
(defn visual-relationship-data
  ...
  (ds-reduce/group-by-column-agg
    ...
    (ds/write! visds "resources/data-discussion.csv")))

(defn visual-relationship
"Visualizing the relationship between investor number and fund amount."  
  [ds]
  (visual-relationship-data ds)
  (with-data (read-dataset "resources/data-discussion.csv" :header true)
    (save (bar-chart :Invest_Amount :Num_Investors_avg
                     :Type :uptake
                     :title "Investment_Investor_Relationship"
                     :group-by :Invest_Amount
                     :x-label "Invest_Amount" :y-label "Num_Investors"
                    :legend true) 
